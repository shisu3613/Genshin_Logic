...
type Server struct {
	...
	MsgHandler ziface2.IMsgHandler

	//链接模块的管理模块
	ConnMgr ziface2.IConnManager

	//该Server创建链接之后自动调用Hook函数 -- OnConnStart
	OnConnStart func(conn ziface2.IConnection)

	//该Server销毁链接之前自动调用Hook函数 -- OnConnStop
	OnConnStop func(conn ziface2.IConnection)
}
...
func (s *Server) SetOnConnStart(hook func(conn ziface2.IConnection)) {
	s.OnConnStart = hook
}

// SetOnConnStop 注册OnConnStop方法
func (s *Server) SetOnConnStop(hook func(conn ziface2.IConnection)) {
	s.OnConnStop = hook
}
